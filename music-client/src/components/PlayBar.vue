  ...
  <div class="play-bar" :class="{show:!toggle}">
    ...
    <div class="kongjian">
      ...
      <div class="playing-speed">
        <!--播放开始时间-->
        <div class="current-time">{{this.nowTime}}</div>
        ...
        <!--播放结束时间-->
        <div class="left-time">{{this.songTime}}</div>
        <!--音量-->
    ...
    next(){
      if (this.listIndex != -1 || this.listOfSongs.length > 1) {  //当前处于不可能状态或者只有一首音乐的时候不执行
        if (this.listIndex < this.listOfSongs.length-1) {                    // 不是最后一首音乐
          this.$store.commit('setListIndex', this.listIndex + 1);   // 直接切换下一首
         
        }else{                                                     //当前是最后一首音乐
           this.$store.commit('setListIndex',0);   //切换第一首
